...
# -*- coding: utf-8 -*-
"""
Wavelet-Enhanced SPI Forecasting System (Q1 Research Grade) - NO CLI VERSION
=============================================================================
Advanced drought forecasting using wavelet analysis and machine learning.

This is the NO-CLI version with hardcoded configuration.
Edit the configuration variables at the top of the file and run directly.
...
Version: 2.0.1 NO CLI (Q1 Ready - Hardcoded Configuration)
License: MIT
"""
...
import json
# import argparse  # NOT NEEDED - No CLI in this version
import logging
import time
import re
...
# ✅ EDIT THESE VALUES FOR YOUR DATA
# This is the NO-CLI version - edit these settings and run the script directly

INPUT_FILE = "C:/Users/ser_o/OneDrive/Documents/veri.xlsx"  # ← Your input data file
OUTPUT_DIR = "C:/Users/ser_o/OneDrive/Documents/sonuclar/"  # ← Output directory
STATION_ID = None  # ← None = all stations, "17030" = single station only

# Additional settings (optional)
SPI_SCALES = [12]  # ← SPI time scales (e.g., [3, 6, 12])
FORECAST_LEADS = [0]  # ← Forecast lead times (0 = concurrent, 1 = 1-month ahead)
FAST_MODE = False  # ← True = faster but fewer combinations
TOP_K_MODELS = 4  # ← Number of top models to analyze
SELECT_BY = "KGE"  # ← Model selection criterion: "KGE" or "RMSE"
ENABLE_CACHING = True  # ← Cache results for faster reruns
VERBOSE_LEVEL = 1  # ← 0=WARNING, 1=INFO, 2=DEBUG
...
# ==================== MAIN EXECUTION ====================
def main():
    """Main entry point - NO CLI version using hardcoded configuration."""

    # ✅ Create config from hardcoded variables (defined at lines 175-190)
    config = ExperimentConfig(
        ...
        fast_mode=FAST_MODE,
        run_all_stations=STATION_ID is None,
    ...
    logger.info("=" * 70)
    logger.info("Wavelet-Enhanced SPI Forecasting System v2.0.1 NO CLI (Q1 Ready)")
    logger.info("=" * 70)
    logger.info(f"Input: {config.input_file}")
    logger.info(f"Output: {config.output_dir}")
    logger.info(f"Station: {config.station_id if config.station_id else 'All stations'}")
    logger.info("=" * 70)
    ...
    if config.station_id:
        if config.station_id not in stations:
            logger.error(f"Station {config.station_id} not found!")
            sys.exit(1)
        stations = {config.station_id: stations[config.station_id]}

    logger.info(f"Processing {len(stations)} station(s)")

    # ✅ PROCESS EACH STATION
    for station_id, station_data in stations.items():
        logger.info("")
        logger.info("=" * 70)
        logger.info(f"PROCESSING: {station_data['label']}")
        logger.info("=" * 70)

        df_precip = station_data["df"]
        logger.info(f"Data range: {df_precip['date'].min()} to {df_precip['date'].max()}")
        logger.info(f"Total months: {len(df_precip)}")

        # ✅ COMPUTE SPI FOR EACH SCALE
        for scale in config.spi_scales:
            ...
            valid_spi = df_spi[spi_col].notna().sum()
            logger.info(f"Valid SPI values: {valid_spi}/{len(df_spi)}")

            if valid_spi < config.min_train_samples:
                logger.warning(f"Insufficient data for station {station_id}, scale {scale} - skipping")
                continue

            # ✅ FEATURE ENGINEERING
            logger.info("Building feature variants...")
            frames = build_variant_frames(df_spi, spi_col, config, logger)
            logger.info(f"Created {len(frames)} feature variants")

            # ✅ APPLY FORECAST LEADS
            for lead in config.forecast_leads:
                logger.info(f"\nForecast lead: {lead} month(s)")

                frames_lead = apply_forecast_lead(frames, lead)

                # Simple analysis: just count valid samples per variant
                logger.info("\nVariant summary:")
                for variant_name, variant_df in frames_lead.items():
                    valid_rows = variant_df.dropna().shape[0]
                    logger.info(f"  {variant_name}: {valid_rows} valid samples")

        logger.info(f"\nCompleted station: {station_data['label']}")
